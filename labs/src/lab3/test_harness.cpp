...
int main(int argc, char* argv[])
{
	...
	// Use gold_bins for your reference result
	...
	//uint32_t PADDED_WIDTH = (INPUT_WIDTH + 128) & 0xFFFFFF80;
	uint32_t *device_input = (uint32_t *)allocateDeviceMemory(INPUT_HEIGHT * INPUT_HEIGHT * sizeof(uint32_t));
	uint32_t *device_bin_32 = (uint32_t *)allocateDeviceMemory(HISTO_HEIGHT * HISTO_WIDTH * sizeof(uint32_t));
	uint8_t *device_bin_8 = (uint8_t *)allocateDeviceMemory(HISTO_HEIGHT * HISTO_WIDTH * sizeof(uint8_t));
	copyToDeviceMemory(device_input, &(input[0][0]), INPUT_HEIGHT*INPUT_HEIGHT*sizeof(uint32_t));
	/* End of setup code */

  /* This is the call you will use to time your parallel implementation */
	TIME_IT("opt_2dhisto", 1000, opt_2dhisto(device_input, INPUT_HEIGHT, INPUT_HEIGHT, device_bin_32, device_bin_8);)

  /* Include your teardown code below (temporary variables, function calls, etc.)*/
  copyToHostMemory(kernel_bins, device_bin_8,  HISTO_WIDTH*HISTO_HEIGHT*sizeof(uint8_t));
	freeDeviceMemory(device_input);
	freeDeviceMemory(device_bin_32);
	freeDeviceMemory(device_bin_8);
